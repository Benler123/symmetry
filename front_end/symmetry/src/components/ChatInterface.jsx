...
function ChatInterface({ userName }) {
  ...
  const handleSendMessage = async () => {
    if (message.trim()) {
      ...
      try {
        const response = await fetch(`http://127.0.0.1:8001/chat_with_knowledge/${encodeURIComponent(message)}`);
        const data = await response.json();
        
        // Assuming 'data' contains the response message
        console.log(data)
        setChatHistory(chatHistory => [...chatHistory, { message: data, isReceived: true }]);
      } catch (error) {
